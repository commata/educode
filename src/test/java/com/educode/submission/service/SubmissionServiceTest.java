...
class SubmissionServiceTest {
    ...
    void submit_passes_all_testcases() {
        ...
        User student = User.builder().id(2L).email("s@test.com").password("x").name("학생").role(UserRole.STUDENT).build();
        Classroom classroom = Classroom.builder().id(11L).name("반").inviteCode("ABC123").educator(educator).build();
        Problem problem = Problem.builder().id(22L).educator(educator).title("A+B").content("본문").timeLimit(1000).memoryLimit(128).build();
        ...
        Assignment assignment = Assignment.builder().id(33L).classroom(classroom).problem(problem).deadline(LocalDateTime.now().plusDays(1)).build();
